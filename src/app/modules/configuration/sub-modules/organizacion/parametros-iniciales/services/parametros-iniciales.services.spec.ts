...
import { ParametrosInicialesService } from './parametros-iniciales.service';

describe('NivelesEducativosServices', () => {

  let sindicatosService   : ParametrosInicialesService;
  ...
  beforeEach( () => {
    TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [ ParametrosInicialesService ]
    });

    sindicatosService = TestBed.inject(ParametrosInicialesService);
  ...
  it('Debe crearse el servicio correctamente', () => {
    expect(ParametrosInicialesService).toBeTruthy();
  });
  ...
  it('Crear nivel educativo (Caso exitoso)', waitForAsync( (done: DoneFn) => {
    ...
    const token = 'xxxxx'
    const query: ParametrosIniciales = {
      "id_empresa": "3",
      "vacacionPorVencer": 1
    }
    const mockResult: any = {
  ...
  it('Eliminar Forma de pago (Caso exitoso)', waitForAsync ((done: DoneFn) => {
    ...
    sindicatosService.getTiposVacacionesPorVender()
      .subscribe((resp) => {
        expect(resp).toEqual(mockResult);
        done();
      });
