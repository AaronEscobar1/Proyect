...
describe('ClasificacionOficialComponent', () => {
  ...
  beforeEach( waitForAsync  (() => {
    ...
    services = TestBed.inject(CategoriesService);
  ...
  it('Abrir modal de Crear Nivel', async () => {
    ...
    app.openModalCreate();
    fixture.detectChanges();
    expect(app.titleForm).toBe('Agregar categoría');
  ...
  it('Abrir modal de Editar Nivel', async () => {
    ...
    expect(app.categoriesSelect).toBe(undefined);
    app.editRow(data);
    fixture.detectChanges();
    expect(app.categoriesSelect).toBe(data);
    expect(app.isEdit).toBeTrue();
    expect(app.titleForm).toBe('Editar categoría');
    expect(app.createModal).toBeTrue();  
  })
  ...
  it('Eliminar Niveles Educativos (caso verdadero)', ()=>{
    ...
    const fakeBackend = httpTestingController.match(`${URL}/${data.codcat}`);
