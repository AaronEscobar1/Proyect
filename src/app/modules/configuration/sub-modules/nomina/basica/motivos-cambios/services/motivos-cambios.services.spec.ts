...
import { MotivosCambiosService } from './motivos-cambios.service';
import { MotivosCambios } from '../interfaces/motivos-cambios.interfaces';

describe('NivelesEducativosServices', () => {

  let formasPagoService   : MotivosCambiosService;
  ...
  beforeEach( () => {
    TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [ MotivosCambiosService ]
    });

    formasPagoService = TestBed.inject(MotivosCambiosService);
  ...
  it('Debe crearse el servicio correctamente', () => {
    expect(formasPagoService).toBeTruthy();
  });
  ...
  it('Crear nivel educativo (Caso exitoso)', waitForAsync( (done: DoneFn) => {
    ...
    formasPagoService.create(query)
      .subscribe((resp) => {
        expect(resp).toEqual(mockResult);
        done();
      });
  ...
  it('Eliminar Forma de pago (Caso exitoso)', waitForAsync ((done: DoneFn) => {
    ...
    formasPagoService.delete(query.codcam)
      .subscribe((resp) => {
        expect(resp).toEqual(mockResult);
        done();
      });
