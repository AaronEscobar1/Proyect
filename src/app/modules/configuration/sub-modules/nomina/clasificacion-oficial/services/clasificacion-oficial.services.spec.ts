...
import { ClasificacionOficialService } from './clasificacion-oficial.service';
import { OfficialClassification } from '../interfaces/clasificacion-oficial.interfaces';

describe('NivelesEducativosServices', () => {

  let clasificacionesoficiales   : ClasificacionOficialService;
  ...
  beforeEach( () => {
    TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [ ClasificacionOficialService ]
    });

    clasificacionesoficiales = TestBed.inject(ClasificacionOficialService);
  ...
  it('Crear nivel educativo (Caso exitoso)', waitForAsync( (done: DoneFn) => {
    ...
    const token = 'xxxxx'
    const query: OfficialClassification = {
      "desofi": "De Dirección",
      "tiprep": "06",
      "codofi": "1"
    }
  ...
  it('Eliminar Forma de pago (Caso exitoso)', waitForAsync ((done: DoneFn) => {
    ...
    clasificacionesoficiales.delete(query.codofi, query.tiprep)
      .subscribe((resp) => {
        expect(resp).toEqual(mockResult);
        done();
      });
