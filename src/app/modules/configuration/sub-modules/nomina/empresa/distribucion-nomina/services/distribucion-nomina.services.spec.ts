...
import { DistribucionNominaService } from './distribucion-nomina.service';

describe('NivelesEducativosServices', () => {

  let clasificacionesoficiales   : DistribucionNominaService;
  ...
  beforeEach( () => {
    TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [ DistribucionNominaService ]
    });

    clasificacionesoficiales = TestBed.inject(DistribucionNominaService);
  ...
  it('Deberia retornar arreglo de Niveles Educativos', waitForAsync ((done: DoneFn) => {
    ...
    clasificacionesoficiales.getAllDistribuciones("93")
      .subscribe((resp) => {
        console.log(resp);
  ...
  it('Deberia retornar Nivel Educativo (Id Existente)', waitForAsync( (done: DoneFn) => {
    ...
    clasificacionesoficiales.getById("93", id)
      .subscribe((resp) => {
        expect(resp).toEqual(mockResult);
        done();
      });
  ...
  it('Crear nivel educativo (Caso exitoso)', waitForAsync( (done: DoneFn) => {
    ...
    const token = 'xxxxx'
    const query: DistribucionNomina = {
      "dessuc": "prueba jose",
      "codctb": "12",
      "codubi": "CCS012",
      "codsuc": "1515",
    ...
    clasificacionesoficiales.create("93", query)
      .subscribe((resp) => {
        expect(resp).toEqual(mockResult);
        done();
      });
  ...
  it('actualizar nivel educativo (Caso exitoso)', waitForAsync ((done: DoneFn) => {
    ...
    clasificacionesoficiales.update("93", query)
      .subscribe((resp) => {
        expect(resp).toEqual(mockResult);
        done();
      });
  ...
  it('Eliminar Forma de pago (Caso exitoso)', waitForAsync ((done: DoneFn) => {
    ...
    clasificacionesoficiales.delete(query)
      .subscribe((resp) => {
        expect(resp).toEqual(mockResult);
        done();
      });
