...
import { MotivosFiniquitoService } from './motivos-finiquito.service';
import { MotivosFiniquito } from '../interfaces/motivos-finiquito.interfaces';

describe('NivelesEducativosServices', () => {

  let motivosFiniquitoService   : MotivosFiniquitoService;
  ...
  beforeEach( () => {
    TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [ MotivosFiniquitoService ]
    });

    motivosFiniquitoService = TestBed.inject(MotivosFiniquitoService);
  ...
  it('Debe crearse el servicio correctamente', () => {
    expect(motivosFiniquitoService).toBeTruthy();
  });
  ...
  it('Deberia retornar arreglo de Niveles Educativos', waitForAsync ((done: DoneFn) => {
    ...
    motivosFiniquitoService.getAll()
      .subscribe((resp) => {
        console.log(resp);
  ...
  it('Deberia retornar Nivel Educativo (Id Existente)', waitForAsync( (done: DoneFn) => {
    ...
    motivosFiniquitoService.getById(id)
      .subscribe((resp) => {
        expect(resp).toEqual(mockResult);
        done();
      });
  ...
  it('Crear nivel educativo (Caso exitoso)', waitForAsync( (done: DoneFn) => {
    ...
    motivosFiniquitoService.create(query)
      .subscribe((resp) => {
        expect(resp).toEqual(mockResult);
        done();
      });
  ...
  it('actualizar nivel educativo (Caso exitoso)', waitForAsync ((done: DoneFn) => {
    ...
    motivosFiniquitoService.update(query)
      .subscribe((resp) => {
        expect(resp).toEqual(mockResult);
        done();
      });
